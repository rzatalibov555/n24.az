...
<script>
    const ACTIONS_LANG = {
        ...
        "important_news_1": "<?= $this->lang->line("important_news_1") ?>",
        "important_news_2": "<?= $this->lang->line("important_news_2") ?>",
        "important_news_3": "<?= $this->lang->line("important_news_3") ?>",
        "important_news_4": "<?= $this->lang->line("important_news_4") ?>",
        "important_news_lent": "<?= $this->lang->line("important_news_lent") ?>",
        "interview": "<?= $this->lang->line("interview") ?>",
        "naxcivan": "<?= $this->lang->line("naxcivan") ?>",
        "zengezur_corridor": "<?= $this->lang->line("zengezur_corridor") ?>"
    };

    // type-lara uyğun rəngləri JS array-də təyin edirik
    const TYPE_COLORS = {
        "daily_news": "info",
        ...
        ajax: {
            ...
            dataSrc: function (json) {
                ...
                json.data.forEach(function (row, idx) {
                    ...
                    // Type badge üçün dinamik rənglər
                    const color = TYPE_COLORS[row.type] ?? "secondary"; // bilinməyən type üçün default
                    const typeText = ACTIONS_LANG[row.type] ?? row.type;
                    row.type = `<span class="badge border border-${color} text-${color}">${typeText}</span>`;
